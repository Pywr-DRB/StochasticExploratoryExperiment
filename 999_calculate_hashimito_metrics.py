...
for model in ['reconstruction', 'stationary_ensemble']:
    ...
    shortage_matrix_dict[model] = shortage_matrix
    duration_matrix_dict[model] = duration_matrix
...
for model, shortage_matrix in duration_matrix_dict.items():
    n_realizations = shortage_matrix.shape[0]
    
    c = 'black' if model == 'obs' else 'blue' if model == 'reconstruction' else 'orange'
    
    for i in range(n_realizations):
        ys = shortage_matrix[i, :]
        ax.plot(xs, ys, linestyle='-', color=c)

ax.set_xlabel('Shortage Percentile')
ax.set_ylabel('Max Annual Shortage Duration (days)')
# ax.set_yscale('symlog')
plt.title('Montague Flow Target')
plt.xlim(0, 100)
plt.legend()
plt.show()
