"""
Fig9alt: Combined Exceedance Rate + Satisficing Heatmap

6-panel figure (3 rows x 2 columns).
  Rows = climate scenarios (Baseline, Mixed Future, Wet Future)
  Left column:  Joint exceedance rate (events/year) per (severity, magnitude) bin
  Right column: Fraction of events avoiding FFMP Drought Emergency

The focal grid cell (shared with Fig10) is highlighted with a white box.

Usage:
    python Fig9alt_exceedance_satisficing.py [ssi_window]
"""
...
def plot_combined_figure(all_data, ssi_window, n_bins=GRID_N_BINS, min_count=1):
    """Create the 3x2 combined figure.
    ...
    min_count : int
        Minimum events per bin to colour (else grey).
    """
    ...
    gs = gridspec.GridSpec(
        ...
        left=0.10, right=0.92, bottom=0.08, top=0.88,
    )
    ...
    for row_idx, did in enumerate(DATASETS):
        ...
        # ── Right column: fraction avoiding emergency ─────────────────
        ...
        from methods.plotting.heatmap import compute_min_storage_grid
    ...
    # -- column titles ------------------------------------------------------
    axes[0, 0].set_title('Exceedance Rate (events yr$^{-1}$)',
    ...
    # Right colorbar: fraction avoiding emergency
    bb_right = axes[0, 1].get_position()
    cbar_ax_frac = fig.add_axes([bb_right.x0, cbar_top, bb_right.width, cbar_h])
    ...
    # -- save ---------------------------------------------------------------
    fname = (f"{FIG_OUTPUT_DIR}/Fig9alt_exceedance_satisficing_ssi{ssi_window}"
...
def main():
    ...
    print(f"Fig9alt: Exceedance + Satisficing (SSI-{ssi_window}, "
