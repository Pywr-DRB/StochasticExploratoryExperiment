...
def plot_duration_boxplot(ax, panel_label='c)', show_historic=True):
    """
    Plot Panel B2: Duration box plot showing distribution of all episode durations.

    X-axis groups: Warning, Watch, Emergency (3 groups)
    Per group: 3 side-by-side box plots (Stationary, Climate Low, Climate High)
    Box plots show the distribution of individual episode durations across all
    realizations (each episode is one data point, not a per-realization mean).
    ...
    for zone_idx, zone_num in enumerate(zone_order):
        for ds_idx, dataset_id in enumerate(SCENARIOS):
            ...
            # All individual episode durations for this zone and dataset
